...
const DeleteDialog = (props) => {
    const { open, fileType, fileModel, onRequestClose, onRequestDelete, onRequestDeleteError } = props;
    ...
    return (
        <Dialog open={open} onClose={onRequestClose} maxWidth={false}>
            <DialogTitle>{i18n.t('Delete {{what}}', { what: getFileTypeLabel(fileType) })}</DialogTitle>
            <DialogContent>
                {i18n.t('This {{what}} will be deleted. Continue?', { what: getFileTypeLabel(fileType) })}
            </DialogContent>
            <DialogActions>
